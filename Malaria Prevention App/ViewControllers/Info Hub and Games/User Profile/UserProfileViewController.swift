...
protocol SaveRemainingPillsProtocol: class {
  func getRemainingPillsValueFrom(textField: UITextField)
}
...
class UserProfileViewController: UIViewController {
  ...
  @IBAction func editPressed(sender: UIButton) {
    ...
    // Save the new medicine stock.
    CoreDataHelper.sharedInstance.saveContext(context!)
    
    // Save the new reminder value.
    UserSettingsManager.UserSetting.pillReminderValue.setString(reminderValue.toString())
    ...
    for intervalValue in PillStatusNotificationsManager.ReminderInterval.allValues {
      let action = UIAlertAction(title: intervalValue.toString(),
                                 ...
                                 handler: {
                                  ...
                                  // Set the reminderValue, but not save it yet.
...
extension UserProfileViewController: SaveRemainingPillsProtocol {
  ...
  func getRemainingPillsValueFrom(textField: UITextField) {
    ...
    // Store new values in Core Data, but don't save them until the user
    // presses "Save" successfully.
